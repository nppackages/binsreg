...
{p 12 12} {opt dots(dotsopt)} {opt dotsgrid(dotsgridoption)} {opt dotsplotopt(dotsoption)}{p_end}
{p 12 12} {opt line(lineopt)} {opt linegrid(#)} {opt lineplotopt(lineoption)}{p_end}
{p 12 12} {opt ci(ciopt)} {opt cigrid(cigridoption)} {opt ciplotopt(rcapoption)}{p_end}
{p 12 12} {opt cb(cbopt)} {opt cbgrid(#)} {opt cbplotopt(rareaoption)}{p_end}
...
{p 12 12} {opt nbins(nbinsopt)} {opt binspos(position)} {opt binsmethod(method)} {opt nbinsrot(#)} {opt samebinsby} {opt randcut(#)}{p_end}
...
{p 12 12} {opt level(level)} {opt usegtools(on/off)} {opt precision(single/double)} {opt noplot} {opt savedata(filename)} {opt replace}{p_end}
...
{p 6 9} 3. Make sure the package {cmd:reghdfe} installed has a version number greater than or equal to 5.9.0 (03jun2020).
...
The default is {opt dotsgrid(mean)}, which corresponds to one dot per bin evaluated at the sample
average of {it:indvar} within each bin (canonical binscatter).
{p_end}
...
{p 4 8} {opt linegrid(#)} specifies the number of evaluation points of an evenly-spaced grid within each bin
used for evaluation of the point estimate set by the {cmd:line(p s)} option.
...
{p 4 8} {opt lineplotopt(lineoption)} standard graphs options to be passed on to the {help twoway:twoway}
command to modify the appearance of the plotted line.
{p_end}
...
{p 4 8} {opt cigrid(cigridoption)} specifies the number and location of evaluation points in the grid used to
construct the confidence intervals set by the {opt ci(p s)} option.
Two options are available: {it:mean} and a {it:numeric} non-negative integer.
...
Both options can be used simultaneously: for example, {opt cigrid(mean 5)} generates six evaluation points
within each bin containing the sample mean of {it:indvar} within each bin and five evenly-spaced points.
The default is {opt cigrid(mean)}, which corresponds to one evaluation point set at the sample average of
{it:indvar} within each bin for confidence interval construction.
{p_end}
...
{p 4 8} {opt polyregcigrid(#)} specifies the number of evaluation points of an evenly-spaced grid within each bin
used for constructing confidence intervals based on polynomial regression set by the {cmd:polyreg(p)} option.
...
{p 4 8} {opt polyregplotopt(lineoption)} standard graphs options to be passed on to the {help twoway:twoway} command
to modify the appearance of the global polynomial regression fit.
{p_end}
...
By default, the binning structure is selected for each subgroup separately, but see the option {cmd:samebinsby} below
for imposing a common binning structure across subgroups.
{p_end}
...
via the companion command {help binsregselect:binsregselect}.
...
{p 4 8} {opt nbinsrot(#)} specifies an initial number of bins value used to 
construct the DPI number of bins selector.
If not specified, the data-driven ROT selector is used instead.
{p_end}

{p 4 8} {opt samebinsby} forces a common partitioning/binning structure across all subgroups 
specified by the option {cmd:by()}.
...
each bin for approximating the supremum (or infimum) operator.
...
{p 4 8} {opt noplot} omits binscatter plotting, but still allows graph data to be saved with {opt savedata()}.
{p_end}

{p 4 8} {opt savedata(filename)} specifies a filename for saving all data underlying the binscatter plot (and more). It may be combined with {opt noplot}. The saved data include {cmd:bin_n}, the number of observations in each bin, and corresponding {cmd:*_n} variables for plotted series with bin identifiers.
{p_end}
