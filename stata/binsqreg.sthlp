...
{p 4 13} {cmdab:binsqreg} {depvar} {it:indvar} [{it:covars}] {ifin} {weight} [ {cmd:,} {opt quantile(#)} {opt deriv(v)} {opt at(position)}{p_end}
{p 13 13} {opt dots(p s)} {opt dotsgrid(dotsgridoption)} {opt dotsplotopt(dotsoption)}{p_end}
{p 13 13} {opt line(p s)} {opt linegrid(#)} {opt lineplotopt(lineoption)}{p_end}
{p 13 13} {opt ci(p s)} {opt cigrid(cigridoption)} {opt ciplotopt(rcapoption)}{p_end}
{p 13 13} {opt cb(p s)} {opt cbgrid(#)} {opt cbplotopt(rareaoption)}{p_end}
...
{p 13 13} {opt nbins(#)} {opt binspos(position)} {opt binsmethod(method)} {opt nbinsrot(#)} {opt samebinsby} {opt randcut(#)}{p_end}
...
{p 13 13} {opt level(level)} {opt usegtools(on/off)} {opt noplot} {opt savedata(filename)} {opt replace}{p_end}
...
Binscatter provides a flexible way of describing the quantile relationship between two variables, after possibly adjusting for other covariates, based on partitioning/binning of the independent variable of interest.
...
Hypothesis testing about the regression function can be conducted via the companion command {help binstest:binstest}. Hypothesis testing about pairwise group comparison can be conducted via the companion command {help binspwc: binspwc}. Binscatter estimation based on the least squares method can be conducted via the command {help binsreg: binsreg}.
{p_end}
...
{p 4 8} {opt quantile(#)} specifies the quantile to be estimated and should be a number between 0 and 1, exclusive.  The default value of 0.5 corresponds to the median.
{p_end}
...
{p 4 8} {cmd:vce(}{it:{help qreg##qreg_vcetype:vcetype}}{cmd:)} specifies the {it:vcetype} for variance estimation used by the command {help qreg##qreg_options:qreg}. Bootstrapping-based VCE can be also be obtained by setting {cmd:vce(boot, reps(#))} where {cmd:reps(#)} specifies the number of bootstrap replications. Weights are not allowed when bootstrapping VCE is specified.
The default is {cmd:vce(robust)}.
{p_end}

{p 4 8} {opt asyvar(on/off)} specifies the method used to compute standard errors.
If {cmd:asyvar(on)} is specified, the standard error of the nonparametric component is used and the uncertainty related to other control variables {it:covars} is omitted. Default is {cmd:asyvar(off)}, that is,
the uncertainty related to {it:covars} is taken into account.
{p_end}
...
{p 8 8} . {stata binsqreg price weight length foreign, quantile(0.5) ci(3 3) cb(3 3)}{p_end}
